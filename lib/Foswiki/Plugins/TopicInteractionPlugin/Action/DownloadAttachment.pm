...
sub handle {
    ...
    unless ( -e $archivePath ) {
        ...
        foreach my $fileName (@fileNames) {
            ...
            unless (
                Foswiki::Func::attachmentExists( $web, $topic, $fileName ) )
            {
                Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                    $response, 104, "Attachment $fileName does not exist",
                    $id );
                return;
            }
            ...
            unless ( $zip->addFile( $pubDir . "/" . $fileName, $fileName ) ) {
                Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                    $response, 106,
                    "Error while adding $fileName to the download archive",
                    $id );
                return;
            }
        }
        ...
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
            "creating zip $archivePath");
        my $status = $zip->writeToFileNamed($archivePath);
        if ( $status != AZ_OK ) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                $response, 107, "Error while creating archive", $id );
            return;
        }
    }
    else {
...
sub getArchiveName {
    ...
    foreach my $fileName (@$fileNames) {
        ...
        unless ($attachment) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
                "warning $fileName not found in attachments hash");
            next;
        }
        $md5->add($fileName);
