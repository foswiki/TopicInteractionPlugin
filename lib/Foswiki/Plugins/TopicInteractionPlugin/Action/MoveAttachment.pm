...
sub handle {
    ...
    # check existence
    ( $newWeb, $newTopic ) =
      Foswiki::Func::normalizeWebTopicName( $newWeb, $newTopic );
    unless ( Foswiki::Func::topicExists( $newWeb, $newTopic ) ) {
        Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC( $response,
            101, "Topic $newWeb.$newTopic does not exist", $id );
        return;
    }
    ...
    if ( $newWeb eq $web && $newTopic eq $topic ) {
        Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC( $response,
            108, "Won't copy attachment to itself", $id );
        return;
    }
    ...
    foreach my $fileName (@fileNames) {
        ...
        unless ( Foswiki::Func::attachmentExists( $web, $topic, $fileName ) ) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                $response, 104, "Attachment $fileName does not exist", $id );
            return;
        }
...
"moving $web.$topic.$fileName to $newWeb.$newTopic.$toAttachment"
            );

            unless (DRY) {
                Foswiki::Func::moveAttachment( $web, $topic, $fileName, $newWeb,
                    $newTopic, $toAttachment );
            }
        }
