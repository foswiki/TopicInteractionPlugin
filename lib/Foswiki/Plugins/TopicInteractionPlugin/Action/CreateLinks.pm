...
sub handle {
    ...
    foreach my $fileName (@fileNames) {
        ...
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
            "createlink fileName=$fileName, web=$web, topic=$topic");

        my ( $meta, $text ) = Foswiki::Func::readTopic( $web, $topic );
        my $prevAttachment = $meta->get( 'FILEATTACHMENT', $fileName );
        my $prevHide =
          ( $prevAttachment && $prevAttachment->{attr} =~ /h/ ) ? 1 : 0;
            ...
            unless (DRY) {
                $error = Foswiki::Func::saveAttachment(
                    ...
                    $fileName,
                    {
                        ...
                        hide    => $prevHide
                        , # SMELL: we need to ship the prev hide flag as it gets nulled otherwise
                        createlink => 1,
                    }
                );
