...
sub handle {
    ...
    unless ( Foswiki::Func::attachmentExists( $web, $topic, $fileName ) ) {
        Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC( $response,
            104, "Attachment $fileName does not exist", $id );
        return;
    }
...
"fileName=$fileName, newFileName=$newFileName, comment=$fileComment, hide=$fileHide, createlink=$fileCreateLink"
    );
    ...
    try {
        unless (DRY) {
            if ( $newFileName ne $fileName ) {
                Foswiki::Func::moveAttachment( $web, $topic, $fileName, $web,
                    $topic, $newFileName );
            }
