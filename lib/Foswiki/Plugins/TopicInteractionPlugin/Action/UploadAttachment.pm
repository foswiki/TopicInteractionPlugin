...
sub handle {
    ...
    my ( $fileName, $origName ) =
      Foswiki::Func::sanitizeAttachmentName( $params->{filename} );

    Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
        "'$origName' has been renamed to '$fileName'")
    ...
    Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
        "receiving file $fileName, chunk $chunk of $nrChunks, id=$id"
          . ( $isMultiPart ? ' in multipart mode' : ' in normal mode' ) );
    ...
    if ($isMultiPart) {
        ...
        unless ( defined $stream ) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                $response, 110, "Stream not found for '$fileName'", $id );
            return;
        }
        my $r;
        my $transfer;
        while ( $r = sysread( $stream, $transfer, 0x80000 ) ) {
            if ( !defined $r ) {
                next if ( $! == Errno::EINTR );
                Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                    $response, 1, "System read error: $!", $id );
                return;
            }
            $data .= $transfer;
        }
    ...
    if ( -e $tmpFileName && $chunk <= $nrChunks ) {
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
            "appending to $tmpFileName");
        appendFile( $tmpFileName, $data );
    }
    else {
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
            "saving to $tmpFileName");
    ...
    if ( $chunk + 1 >= $nrChunks ) {
        ...
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
            "finished uploading $newFileName");
        ...
        if ($prevAttachment) {
            ...
            Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
                "prevComment=$prevComment, prevHide=$prevHide");
        }
        else {
            Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
                "no previous FILEATTACHMENT for $fileName");
        }
        ...
        unless ($fileSize) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                $response, 111, "Zero-sized file upload of '$fileName'", $id );
            return;
        }
        ...
        Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
"fileSize=$fileSize, maxSize=$maxSize, fileDate=$fileDate, fileComment=$fileComment, fileHide=$fileHide, fileCreateLink=$fileCreateLink"
        );

        if ( $maxSize && $fileSize > $maxSize * 1024 ) {
            Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC(
                $response, 109, "Oversized upload of '$fileName'", $id );
            return;
        }
        ...
        try {
            Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
                "attaching $fileName to $web.$topic");
            unless (DRY) {
            ...
            Foswiki::Plugins::TopicInteractionPlugin::Core::writeDebug(
                "removing temp file $newFileName");
            unlink $newFileName if -e $newFileName;

            sleep(1)
              ; # sleep for a while to prevent a firing hurdle of events on save handlers in other extensions
    ...
    Foswiki::Plugins::TopicInteractionPlugin::Core::printJSONRPC( $response, 0,
        { $origName => $fileName }, $id );
}
