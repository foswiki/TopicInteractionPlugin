...
#%META:FILEATTACHMENT{name="cross06.jpg" attachment="cross06.jpg" attr="" comment="" date="1287484667" size="30247" user="micha" version="1"}%
    ...
    $index = 0;
    my $webDAVLinkPluginEnabled =
      Foswiki::Func::getContext()->{WebDAVLinkPluginEnabled};
    my $webDAVFilter = $Foswiki::cfg{TopicInteractionPlugin}{WebDAVFilter}
      || qr/((xlt|xls|ppt|pps|pot|doc|dot)(x|m)?)|odc|odb|odf|odg|otg|odi|odp|otp|ods|ots|odt|odm|ott|oth|mpp/;
    ...
    foreach my $attachment (@selectedAttachments) {
        ...
        my $webDavAction =
            '<a rel="nofollow" href="'
          . $webDavUrl . '" '
          . 'title="%MATETEXT{"edit this attachment" args="<nop>'
          . $info->{name} . '"}%">'
          . '%MAKETEXT{"edit"}%</a>';
        ...
        if ( $theFormat =~ /\$oldversions/ && $info->{version} > 1 ) {
            ...
            for ( my $i = $info->{version} - 1 ; $i > 0 ; $i-- ) {
                my ( $date, $user, $rev, $comment ) =
                  Foswiki::Func::getRevisionInfo( $thisWeb, $thisTopic, $i,
                    $info->{name} );
...
# use webdav urls for document types that are webdav-enabled via WebDAVLinkPlugin
        if (   $webDAVLinkPluginEnabled
            && $info->{name} =~ /\.($webDAVFilter)$/i )
        {
...
sub getAttachmentInfo {
    ...
    my %info = (
        ...
        user =>
          ( $attachment->{user} || $attachment->{author} || 'UnknownUser' ),
        size    => $size,
        sizeK   => sprintf( "%.2f", $size / 1024 ),
        sizeM   => sprintf( "%.2f", $size / ( 1024 * 1024 ) ),
        comment => ( defined $attachment->{comment} )
        ? $attachment->{comment}
        : '',
    ...
    ( $info{userWeb}, $info{userTopic} ) =
      Foswiki::Func::normalizeWebTopicName( '', $info{user} );
...
sub renderPager {
    ...
    $lastPage = POSIX::ceil( $totalEntries / $entriesPerPage ) - 1
      if $entriesPerPage;
    $currentPage = POSIX::floor( $firstEntry / $entriesPerPage )
      if $entriesPerPage;
...
#print STDERR "entriesPerPage=$entriesPerPage, totalEntries=$totalEntries, currentPage=$currentPage, firstEntry=$firstEntry, lastPage=$lastPage\n";
    ...
    if ( $currentPage > 0 ) {
        ...
        $result .=
"<a href='#skip$skip' class='natAttachmentsPagerPrev {skip:$skip}'>%MAKETEXT{\"Previous\"}%</a>";
    }
    else {
        $result .=
"<span class='natAttachmentsPagerPrev foswikiGrayText'>%MAKETEXT{\"Previous\"}%</span>";
    }
    ...
    for ( my $i = $startPage ; $i <= $endPage ; $i++ ) {
        ...
        $result .= "<a href='#skip$skip' class='$marker {skip:$skip}'>"
          . ( $i + 1 ) . "</a>";
        $count++;
    }
    ...
    if ( $currentPage < $lastPage ) {
        my $skip = ( $currentPage + 1 ) * $entriesPerPage;
        $result .=
"<a href='#skip$skip' class='natAttachmentsPagerNext {skip:$skip}'>%MAKETEXT{\"Next\"}%</a>";
    }
    else {
        $result .=
"<span class='natAttachmentsPagerNext foswikiGrayText'>%MAKETEXT{\"Next\"}%</a>";
    }
